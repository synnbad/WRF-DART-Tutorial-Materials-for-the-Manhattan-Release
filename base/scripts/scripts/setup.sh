...
# Create directories
for dir in "${RUN_DIR}" "${TEMPLATE_DIR}" "${OBSPROC_DIR}" "${OUTPUT_DIR}" "${ICBC_DIR}" \
           "${POST_STAGE_DIR}" "${OBS_DIAG_DIR}" "${PERTS_DIR}" "${SHELL_SCRIPTS_DIR}" \
           "${RUN_DIR}/WRFIN" "${RUN_DIR}/WRFOUT" "${RUN_DIR}/WRF_RUN"; do
    if [ ! -d "$dir" ]; then
        mkdir -p "$dir" || { echo "ERROR: unable to make directory '$dir'"; exit 2; }
    fi
done

# Put DART executables in proper place
for exe in advance_time filter pert_wrf_bc obs_diag obs_sequence_tool \
           obs_seq_to_netcdf fill_inflation_restart wrf_dart_obs_preprocess; do
...
if [ ! -e "${RUN_DIR}/WRF_RUN/wrf.exe" -o ! -e "${RUN_DIR}/WRF_RUN/real.exe" ]; then
